...
std::string HelpMessage()
{
    ...
    strUsage += "  -port=<port>           " + _("Listen for connections on <port> (default: 66644)") + "\n";
    ...
    strUsage += "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 44466)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeB3Coinamount=<n> " + _("Keep N B3Coin anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. B3Coin is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. B3Coin is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("B3Coin version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "B3Coin server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of B3Coin") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart B3Coin to complete") << "\n";
    ...
    nAnonymizeB3CoinAmount = GetArg("-anonymizeB3Coinamount", 0);
    if(nAnonymizeB3CoinAmount > 999999) nAnonymizeB3CoinAmount = 999999;
    if(nAnonymizeB3CoinAmount < 2) nAnonymizeB3CoinAmount = 2;
    ...
    LogPrintf("Anonymize B3Coin Amount %d\n", nAnonymizeB3CoinAmount);
