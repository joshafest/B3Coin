...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeB3CoinAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeB3CoinAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeB3CoinAmount = strAnonymizeB3CoinAmount.remove(strAnonymizeB3CoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeB3CoinAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeB3CoinAmount*COIN) nMaxToAnonymize = nAnonymizeB3CoinAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeB3CoinAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeB3CoinAmount));
        strAnonymizeB3CoinAmount = strAnonymizeB3CoinAmount.remove(strAnonymizeB3CoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeB3CoinAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
