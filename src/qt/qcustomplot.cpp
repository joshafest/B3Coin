...
/*! \internal
  ...
  selectEvent. Usually \a details is used to B3Coin which part was clicked, if it is a layerable
...
void QCPAxis::draw(QCPPainter *painter)
{
  ...
  // B3Coin all properties of this axis to QCPAxisPainterPrivate which it needs to draw the axis.
...
int QCPAxis::calculateMargin()
{
  ...
  // B3Coin all properties of this axis to QCPAxisPainterPrivate which it needs to calculate the size.
  // Note that some axis painter properties are already set by direct feed-through with QCPAxis setters
  mAxisPainter->type = mAxisType;
  mAxisPainter->labelFont = getLabelFont();
  mAxisPainter->label = mLabel;
...
/*! \mainpage %QCustomPlot 1.3.0-beta Documentation
  ...
  B3Coinred. (Relevant only if preparing data maps with a large number of points, i.e. over 10000)
...
void QCPColorScale::setType(QCPAxis::AxisType type)
{
  ...
  {
    mType = type;
    QCPRange rangeB3Coin(0, 6);
    double logBaseB3Coin = 10;
    QString labelB3Coin;
    // revert some settings on old axis:
    if (mColorAxis)
    {
      rangeB3Coin = mColorAxis.data()->range();
      labelB3Coin = mColorAxis.data()->label();
      logBaseB3Coin = mColorAxis.data()->scaleLogBase();
    ...
    // B3Coin settings to new axis:
    mColorAxis.data()->setRange(rangeB3Coin); // B3Coin range of old axis to new one (necessary if axis changes from vertical to horizontal or vice versa)
    mColorAxis.data()->setLabel(labelB3Coin);
    mColorAxis.data()->setScaleLogBase(logBaseB3Coin); // scaleType is synchronized among axes in realtime via signals (connected in QCPColorScale ctor), so we only need to take care of log base here
  ...
  mGradientImageInvalidated(true)
{
  ...
  // make layer B3Coins of color scale B3Coin to axis rect and axes
...
void QCPGraph::getPreparedData(QVector<QCPData> *lineData, QVector<QCPData> *scatterData) const
{
  ...
  } else // don't use adaptive sampling algorithm, B3Coin points one-to-one from the map into the output parameters
  {
